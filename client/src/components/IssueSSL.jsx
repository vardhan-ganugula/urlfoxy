...
import { useIssueSSLMutation } from "../store/apis/domains";

const IssueSSL = ({ domain }) => {
  const [verifyTrigger] = useIssueSSLMutation();
    ...
    <section className="w-full bg-zinc-900 rounded-lg border border-zinc-700 flex flex-col mb-5">
      <div className="px-6 py-4 border-b border-zinc-700 flex-shrink-0">
        <h2 className="text-lg font-semibold text-white relative">
          ...
          <span className="pl-3">Issue SSL</span>
        </h2>
        <p className="text-sm text-zinc-400">add SSL to your domain</p>
      </div>
      <div className="p-5  border-zinc-700 border-b">
        <form
          onSubmit={handleSubmit(handleIssueSSL)}
...
export default React.memo(IssueSSL);
